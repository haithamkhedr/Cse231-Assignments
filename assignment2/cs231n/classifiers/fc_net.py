...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    ############################################################################
    a,cache=affine_relu_forward(X, self.params['W1'], self.params['b1'])
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for i in range(self.num_layers):
        ...
        gamma ='gamma' + str(i+1)
        beta = 'beta' +str(i+1)
        self.params[w]=np.random.normal(scale=weight_scale,size=(j,H))
  ...
  def loss(self, X, y=None):
    ...
    for i in range(self.num_layers-1):
        if self.use_batchnorm:  
            weights=(self.params['W' +str(i+1)],self.params['b' +str(i+1)],self.params['gamma' +str(i+1)],self.params['beta' +str(i+1)])
            temp,cache=affine_batchnorm_relu_forward(temp, weights,self.bn_params[i])
        else:
            temp,cache=affine_relu_forward(temp, self.params['W' +str(i+1)],self.params['b' +str(i+1)])
        if self.use_dropout:
            temp,mask = dropout_forward(temp , self.dropout_param)
            masks.append(mask)
    ...
    scores,cache=affine_forward(temp,self.params[w],self.params[b])
    caches.append(cache)
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    for i in range(self.num_layers):
        ...
        if(i==0):
            dout,dw,db=affine_backward(dout,caches[j])
        else:
            if self.use_dropout:
                dout = dropout_backward(dout , masks[j])
            if self.use_batchnorm:
                dout,dw,db,dgamma,dbeta =affine_batchnorm_relu_backward(dout,caches[j])
